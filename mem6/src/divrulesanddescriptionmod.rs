...
use dodrio::{bumpalo, bumpalo::Bump, Node, Render, RenderContext};
...
impl Render for RulesAndDescription {
    ...
    fn render<'a>(&self, cx: &mut RenderContext<'a>) -> Node<'a> {
        ...
        <div>
            <h4>
                {text_with_br_newline(GAME_DESCRIPTION,cx.bump)}
            </h4>
             <h6>
                ...
                <a href= "https://github.com/LucianoBestia/mem6_game" target="_blank" style="color:#74bbfb">
                    {vec![text(bumpalo::format!(in cx.bump,
            ...
            <h2>
            {vec![text(
                bumpalo::format!(in cx.bump, "mem6 game rules: {}", "").into_bump_str(),
            )]}
            </h2>
            <h4>
                {text_with_br_newline(GAME_RULES1, cx.bump)}
            </h4>

            {divfullscreenmod::div_fullscreen(self.is_fullscreen,cx.bump)}

            <h4>
                {text_with_br_newline(GAME_RULES2, cx.bump)}
            </h4>
