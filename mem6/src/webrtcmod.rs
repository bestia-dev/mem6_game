...
pub fn web_rtc_start(rrc: &mut RootRenderingComponent, vdom: VdomWeak) {
    websysmod::debug_write("web_rtc_start()");
    let wrtc = &mut rrc.web_rtc_data;
    wrtc.rtc_receiver_ws_uid = websysmod::get_input_element_value_usize_by_id("receiver_ws_uid");
    if let Ok(pc) = web_rtc_new_connection() {
        ...
        web_rtc_setup_on_local_ice_candidate(vdom.clone(), &mut pc);
        let data_channel = web_rtc_new_data_channel_with_callbacks(vdom.clone(), &pc);
...
fn web_rtc_new_data_channel_with_callbacks(
    ...
    web_rtc_dc_setup_on_msg(vdom.clone(), &mut data_channel);
    web_rtc_dc_setup_on_open(vdom.clone(), &mut data_channel);
    //return
    data_channel
}
                ...
                unwrap!(
                    vdom_on_next_tick
                        .with_component({
                            ...
                            move |root| {
                                let rrc = root.unwrap_mut::<RootRenderingComponent>();
                                rrc.web_rtc_data.rtc_is_data_channel_open = true;
...
fn web_rtc_send_offer(rrc: &RootRenderingComponent, receiver_ws_uid: usize, sdp: String) {
    ...
    let msg = websocketmod::WsMessageForReceivers {
        msg_sender_ws_uid: rrc.web_rtc_data.rtc_my_ws_uid,
...
fn web_rtc_send_answer(rrc: &RootRenderingComponent, receiver_ws_uid: usize, sdp: String) {
...
/// send offer over websocket to establish peer connection
fn web_rtc_send_ice_candidates(rrc: &mut RootRenderingComponent, receiver_ws_uid: usize) {
